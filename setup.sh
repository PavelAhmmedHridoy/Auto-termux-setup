...
case $path_choice in
    ...
    2)
        ...
        ;;
    3)
        echo -e "\n${P}[!] UI Mode: Installing Styling Core Only...${N}"
        apt update -y && pkg install git curl zsh eza -y
        ;;
    *)
        echo -e "\n${R}Invalid choice. Please rerun the script.${N}"
        exit 1
        ;;
esac

# 4. UNIVERSAL UI STYLING
...
if [[ ! -d "$HOME/termux-style" ]]; then
    ...
    cd - > /dev/null
    rm -rf "$HOME/termux-style" # Cleanup style folder
else
    termux-style < /dev/tty
fi

# 5. COMMAND PERSISTENCE & SHELL
...
echo -e "       ${R}CLEANUP: setup.sh & style folder REMOVED${N}"
echo -e "${G}────────────────────────────────────────────────────────────${N}"
